...
class ServerCall extends EventEmitter {
  ...
  async deserializeMessage (bytes) {
    const receivedMessage = await this.filter.receiveMessage(bytes);
...
function onStreamClose () {
  // `this` is bound to the Call instance, not the stream itself.
  this.cancelled = this.stream.rstCode === Http2.constants.NGHTTP2_CANCEL;
