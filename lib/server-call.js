...
function addTrailersHandler (call) {
  call.stream.once('wantTrailers', () => {
    let trailersToSend = {
      [kGrpcStatusHeader]: call.status.code,
      [kGrpcMessageHeader]: call.status.details
    };
    const metadata = call.status.metadata;

    if (call.status.metadata) {
      trailersToSend = Object.assign(trailersToSend, metadata.toHttp2Headers());
    }

    call.stream.sendTrailers(trailersToSend);
  });
}
